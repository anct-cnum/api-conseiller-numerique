...
const insertDailyCrasStatsByConseiller = async (db, query, logger, dateDebut) => {
  ...
  detailsCras[0]?.listConseiller.forEach(conseiller => {
    promises.push(new Promise(async resolve => {
      ...
      const queryUpd = { conseiller: new DBRef('conseillers', new ObjectId(conseiller.idConseiller), database), updateAt: dateDebut };
      ...
      if (conseillerStats) {
        ...
        if (conseillerStats.updateAt >= dateDebut) {
        ...
        //Si trouvé alors on additionne la nouvelle stat
        newStat = oldStat ? oldStat + conseiller.totalCras : conseiller.totalCras;
