...
execute(__filename, async ({ logger, db }) => {
  ...
  conseillers.forEach(conseillerId => {
    promises.push(new Promise(async resolve => {
      const cn = await db.collection('conseillers').findOne({ _id: conseillerId });
      const crasCount = await db.collection('cras').find({ 'conseiller.$id': conseillerId, 'structure.$id': { '$exists': false } }).toArray();
      ...
      if (!cn) {
        const cnRupture = (await db.collection('conseillersSupprimes').findOne({ 'conseiller._id': conseillerId }))?.conseiller;
        idPGSA = (await db.collection('structures').findOne({ _id: cnRupture.ruptures.slice(-1)[0]?.structureId }))?.idPG;
        // eslint-disable-next-line max-len
        file.write(`Profil SUPPRIMER;${cnRupture?.statut ?? ''};email inexistant;${await cnRupture?.ruptures.map(d => `${dayjs(d?.dateRupture).format('DD/MM/YYYY')} - ${d?.motifRupture}`)};${idPGSA};${crasCount?.length} CRAS;${dateCra};\n`);
      } else {
