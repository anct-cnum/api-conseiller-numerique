    ...
    readCSV(program.csv).then(async conseillers => {
      conseillers.forEach(conseiller => {
        promises.push(new Promise(async resolve => {
          ...
          // eslint-disable-next-line max-len
          file.write(`${conseillerId};${conseiller['Id ou email CNFS']};${conseiller['Prénom']};${conseiller['Nom']};${conseiller['Raison sociale']};${conseiller['Commune']};${conseiller['Département']};${conseiller['Date de fin de formation']};${conseiller['Lot']};${conseiller['Parcours']};${conseiller['Palier PIX']};${conseiller['ID structure']};${conseiller['Type de structure']};${cnfs?.mattermost?.id ?? 'COOP non activé'}\n`);
          resolve();
        }));
      });
      resolve();
