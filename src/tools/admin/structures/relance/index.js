...
execute(__filename, async ({ logger, db, app, emails, Sentry }) => {
  ...
  try {
    ...
    if (stats.total > 0) {
      logger.info(`[STRUCTURE] Des emails de relance de création de compte ont été envoyés à des structures :  ` +
    ...
    logger.info(`[STRUCTURE] Une erreur est survenue lors de l'envoi des emails de relance de création de compte aux structures : ` +
        `${stats.sent} envoyés / ${stats.error} erreurs`);
