...
execute(__filename, async ({ logger, db }) => {
  ...
  let structureIds = crasStructureId.filter(i => !structures.includes(i)); // liste Id structure inconnu
  let promises = [];

  structureIds = structureIds.map(id => new ObjectId(id));
  logger.info(`Correction des ${structureIds.length} structureId inconnus...`);
  structureIds.forEach(structureId => {
    promises.push(new Promise(async resolve => {
      ...
      if (countCras === 1) {
        if (conseiller?.statut === 'RECRUTE' && !conseiller?.ruptures) {
        ...
        } else if (conseiller?.statut === 'RECRUTE' && conseiller?.ruptures) {
        ...
        } else if (conseiller?.statut === 'RUPTURE' && conseiller?.ruptures.length === 1) {
          ...
          logger.error(`Error statut CN inconnu: Conseiller idPG ${conseiller?.idPG} en erreur ! (${countCras})`);
        }
      } else {
        // Cas qui ne devrait pas y avoir normalement...
        logger.error(`Error cras > 1 : Il y a ${countCras} CRAS qui ont l'id ${structureId} (conseiller ${ArrayCras[0]?.conseiller?.oid})`);
      }
      resolve();
    }));
  });
