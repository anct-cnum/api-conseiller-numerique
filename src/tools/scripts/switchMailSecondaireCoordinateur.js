...
execute(__filename, async ({ logger, db }) => {
  const coordinateurs = await db
  .collection('conseillers')
  .find({ statut: 'RECRUTE', estCoordinateur: true })
  .toArray();
  ...
  logger.info(
    'Remplacer l\'identifiant prenom.nom@conseiller-numerique.fr par le mail secondaire ou personnelle...'
  );
  ...
  coordinateurs.forEach(conseiller => {
    promises.push(
      new Promise(async resolve => {
        ...
        if (!user) {
          logger.error(
            `Coordinateur id: ${conseiller.idPG} (${conseiller.emailCN.adress}) non trouvé dans la collection user !`
          );
          return resolve();
        }
        ...
        .collection('users')
        .findOne({ name: query.email });
        if (checkUserExist) {
          logger.error(
            `Coordinateur id: ${conseiller.idPG} (${conseiller.emailCN.adress}) : conflit, email déjà existante => ${query.email}`
          );
          return resolve();
        }
        await db
        ...
        .updateOne(
          { _id: user._id },
          { $set: { name: query.email }, $unset: { sub: '' } }
        );
        count++;
        resolve();
      })
