    ...
    send: async conseiller => {
      ...
      let onSuccess = () => {
        return db.collection('conseillers').updateOne(
          ...
          {
            $set: {
              'groupeCRAHistorique.$.mailSendM+1,5': true,
              'groupeCRAHistorique.$.dateMailSendM+1,5': new Date()
            },
            $unset: {
      ...
      let onError = async err => {
        await db.collection('conseillers').updateOne(
          ...
          {
            $set: {
              'groupeCRAHistorique.$.mailErrorM+1,5': 'smtpError',
              'groupeCRAHistorique.$.mailErrorDetailM+1,5': err.message
            }
          });
