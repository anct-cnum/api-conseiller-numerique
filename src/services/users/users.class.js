  ...
  constructor(options, app) {
    ...
    app.patch('/candidat/updateInfosCandidat/:id', async (req, res) => {
      ...
      app.get('mongoClient').then(async db => {
        ...
        const schema = Joi.object({
          ...
          .error(
            new Error(
              'Le format du téléphone est invalide, il doit contenir 10 chiffres '
            )
          ),
          // eslint-disable-next-line max-len
          dateDisponibilite: Joi.date().error(
            new Error(
              'La date est invalide, veuillez choisir une date supérieur ou égale à la date du jour'
            )
          ),
          email: Joi.string()
          .email()
        ...
        const userConnected = await app.service('users').Model.findById({ _id: idUser });
      
        const id = userConnected?.entity?.oid;
        ...
        try {
          ...
          .updateMany(
            { 'conseiller.$id': id },
            { $set: changeInfosMisesEnRelation }
          );
        } catch (err) {
        ...
        if (nouveauEmail !== userConnected.data[0].name) {
          ...
          .service('users').Model
          .find({ name: nouveauEmail });
          if (verificationEmail !== 0) {
            logger.error(`Erreur: l'email ${nouveauEmail} est déjà utilisé.`);
            res.status(409).send(
              new Conflict('Erreur: l\'email saisi est déjà utilisé', {
                nouveauEmail,
              }).toJSON()
            );
            return;
          ...
          try {
            ...
            .service('users').Model
            .findById({ _id: new ObjectID(idUser) });
    ...
    app.patch('/candidat/confirmation-email/:token', async (req, res) => {
      ...
      try {
        await app
        .service('users').Model
        .findByIdAndUpdate({ _id: user._id }, { $set: { name: userInfo.mailAModifier } });
        await app
        .service('conseillers')
        .patch(userInfo?.entity?.oid, { email: userInfo.mailAModifier });
      } catch (err) {
        app.get('sentry').captureException(err);
        logger.error(err);
      }
      try {
        const { idPG } = await app
        .service('conseillers')
        .get(userInfo?.entity?.oid);
        await pool.query(
          `UPDATE djapp_coach
            SET email = $2
                WHERE id = $1`,
    ...
    app.patch('/confirmation-email/:token', async (req, res) => {
      ...
      if (userInfo.mailAModifier === undefined) {
        res.status(400).send(
          new BadRequest('le nouveau mail n\'est pas renseignée', {
            token,
          }).toJSON()
        );
        return;
      ...
      try {
        await app
        .service('users').Model
        .findByIdAndUpdate({ _id: userInfo._id }, {
    ...
    app.patch('/users/sendEmailUpdate/:id', async (req, res) => {
      ...
      app.get('mongoClient').then(async db => {
        ...
        if (verificationEmail.length > 0) {
          logger.error(
            `Erreur: l'email ${nouveauEmail} est déjà utilisé par une autre structure`
          );
          res.status(409).send(
            new Conflict(
              'Erreur: l\'email est déjà utilisé par une autre structure',
              {
                nouveauEmail,
              }
            ).toJSON()
        ...
        try {
          await app
          .service('users').Model
          .findByIdAndUpdate({ _id: idUser }, { $set: { token: uuidv4(), mailAModifier: nouveauEmail } });
          const user = await app
          .service('users').Model
          .findById({ _id: new ObjectID(idUser) });
    ...
    app.get('/users/verifyToken/:token', async (req, res) => {
      ...
      const user = await app.service('users').Model.findOne({ token });
      ...
      const { roles, name, persoEmail, nom, prenom, support_cnfs } =
      user;
      if (roles.includes('conseiller')) {
        ...
        .service('conseillers')
        .get(user.entity?.oid);
        user.mailAModifier = conseiller.email;
    ...
    app.post('/users/inviteAccountsPrefet', async (req, res) => {
      ...
      if (!departement && !regionCode) {
        ...
        if (departement) {
          const schemaDeparetement = await validationCodeDepartement(Joi)(
            niveau
          );
      ...
      let promises = [];
      const errorConflict = email =>
        res
        .status(409)
        .send(
          new Conflict(
            `Compte déjà existant pour l'email : ${email}, veuillez le retirer de la liste`
          )
        );
      const errorBadRequestJoi = schema =>
      ...
      await emails.forEach(async email => {
        ...
        await app.service('users').Model.create(userInfo);
      });
    ...
    app.post('/users/inviteStructure', async (req, res) => {
      ...
      app.get('mongoClient').then(async db => {
        ...
        if (verificationEmail !== 0) {
          ...
          .send(
            new Conflict(
              'Erreur: l\'email est déjà utilisé pour une structure'
            ).toJSON()
          );
          return;
        }
        const emailExistStructure = await db
        ...
        try {
          ...
          await app.service('users').Model.create(newUser);
          let mailer = createMailer(app, email);
          const emails = createEmails(db, mailer);
          let message = emails.getEmailMessageByTemplateName(
            'invitationCompteStructure'
          );
          await message.send(newUser, email);
          let messageCoop = emails.getEmailMessageByTemplateName(
            'invitationStructureEspaceCoop'
          );
          await messageCoop.send(newUser);

          res.status(200).send({ status: 'Invitation à rejoindre la structure envoyée !' });
        } catch (error) {
          context.app.get('sentry').captureException(error);
          logger.error(error);
    ...
    app.get('/users/listByIdStructure/:id', async (req, res) => {
      try {
        ...
        const users = await app.service('users').Model
        .aggregate([
          { $match: { 'entity.$id': new ObjectId(structureId) } },
          { $project: { name: 1, roles: 1, passwordCreated: 1 } },
        ]);
        res.status(200).json(users);
      } catch {
        res.status(404).send(
          new NotFound('Aucun utilisateur n\'est disponible dans cette structure', {
          }).toJSON()
        );
      }
    });
    ...
    app.post('/users/choosePassword/:token', async (req, res) => {
      ...
      if (typeEmail === 'bienvenue' && role === 'conseiller') {
        app.get('mongoClient').then(async db => {
          ...
          .service('users').Model
          .findByIdAndUpdate({ _id: user._id },
            { $set: { token: uuidv4(), name: email } });
          user.name = email;
          // La boite mail a été créée dans import-recrutes.js
          await updateMailboxPassword(
            gandi,
            user.entity.oid,
            login,
            password,
        ...
        try {
          ...
          if (typeEmail === 'bienvenue') {
            switch (role) {
              case 'admin':
                message = emails.getEmailMessageByTemplateName(
                  'bienvenueCompteAdmin'
                );
                await message.send(user);
                break;
              case 'structure':
                message = emails.getEmailMessageByTemplateName(
                  'bienvenueCompteStructure'
                );
                await message.send(user);
                break;
              case 'prefet':
                message = emails.getEmailMessageByTemplateName(
                  'bienvenueComptePrefet'
                );
                await message.send(user);
                break;
              case 'candidat':
                message = emails.getEmailMessageByTemplateName(
                  'bienvenueCompteCandidat'
                );
                await message.send(user);
                break;
              case 'hub_coop':
                message =
                  emails.getEmailMessageByTemplateName('bienvenueCompteHub');
                await message.send(user);
                break;
              default:
                break;
            ...
            if (adressCN === undefined) {
              logger.error(
                `AdressCN not found for conseiller id id=${conseiller._id}`
              );
              res
              .status(404)
              .send(
                new NotFound(
                  'Adresse email Conseiller Numérique non trouvée'
                ).toJSON()
              );
              return;
            }
    ...
    app.post('/users/checkForgottenPasswordEmail', async (req, res) => {
      ...
      if (user.roles.includes('conseiller') && user.passwordCreated === false) {
        ...
        res.status(409).send(
          new Conflict(
            `Vous n'avez pas encore activé votre compte. Pour cela, 
            cliquez sur le lien d'activation fourni dans le mail ayant pour objet "Activer votre compte Coop des Conseillers numériques France Services"`,
            {
              username,
            }
          ).toJSON()
      ...
      if (user.roles[0] === 'conseiller') {
        const hide = t => {
          if (t.length === 0) {
            ...
            return (
              t.charAt(0) + '*'.repeat(t.length - 2) + t.charAt(t.length - 1)
            ); // abcdef => a****f
          }
        };
        ...
        if (match && match.length > 3) {
          ...
          const err = new Error(
            'Erreur mot de passe oublié, format email invalide'
          );
          logger.error(err);
          app.get('sentry').captureException(err);
    ...
    app.post('/users/sendForgottenPasswordEmail', async (req, res) => {
      ...
      if (user.passwordCreated === false) {
        res.status(400).send(
          new BadRequest('Error authorization forgottenPassword', {
            username,
          }).toJSON()
        );
        return;
      ...
      try {
        this.Model.updateOne(
          { _id: user._id },
          { $set: { token: user.token, tokenCreatedAt: new Date() } }
        );
    ...
    app.patch('/users/changement-email-pro/:token', async (req, res) => {
      ...
      if (total === 0) {
        res.status(404).send(
          new NotFound('Compte non trouvé', {
            id: user._id,
          }).toJSON()
        );
        return;
      ...
      if (!user?.roles.includes('conseiller') || !user?.support_cnfs) {
        res.status(409).send(
          new Conflict('Vous n\'avez pas l\'autorisation', {
            id: user._id,
          }).toJSON()
        );
        return;
      ...
      app.get('mongoClient').then(async db => {
        ...
        const lastLogin = conseiller?.emailCN?.address.substring(
          0,
        ...
        const message = emails.getEmailMessageByTemplateName(
          'confirmationChangeEmailCnfs'
        );
        ...
        if (conseiller?.emailCN?.address) {
          await deleteMailbox(
            ...
            logger,
            Sentry
          )(conseillerId, lastLogin)
          .then(async () => {
            // eslint-disable-next-line max-len
            return patchApiMattermostLogin({
              Sentry,
              logger,
              db,
              mattermost,
            })({ conseiller, userIdentity });
          })
          .then(() => {
          ...
          .then(async () => {
            await misesAJourPg(pool)(
              conseiller.idPG,
              user.support_cnfs.nom,
              user.support_cnfs.prenom
            );
            return await misesAJourMongo(db, app)(
              conseillerId,
              email,
              userIdentity,
              password
          ...
          .then(async () => {
            try {
              await createMailbox({ gandi, db, logger, Sentry })({
                conseillerId,
                login,
                password,
              });
              ...
              return res.status(200).send({
                messageCreationMail:
                    'Votre nouvel email a été créé avec succès',
              });
            } catch (error) {
              logger.error(error);
              app.get('sentry').captureException(error);
              ...
              .send(
                new GeneralError(
                  'Erreur lors de la création de la boîte mail, veuillez contacter le support'
                )
              );
              return;
            }
