...
const validateExportStatistiquesSchema = exportTerritoiresInput => Joi.object({
  ...
  codePostal: Joi.string().required().allow('null', '').error(new Error('Le code postal est invalide')),
  ville: Joi.string().required().allow('null', '').error(new Error('La ville est invalide')),
  codeCommune: Joi.string().required().allow('null', '').error(new Error('Le codeCommune est invalide')),
