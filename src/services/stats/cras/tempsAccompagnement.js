const getStatsTempsAccompagnement = async (db, query) => {
  //Gestion des nombres >= 60
  let tempsAccompagnement = await db.collection('cras').aggregate(
    [
      { $unwind: '$cra.duree' },
  ...
  if (tempsAccompagnement?.length === 0) {
    tempsAccompagnement = [
  ...
  tempsAccompagnement.forEach(async activite => {
    ...
    if (dureesString?.length > 0) {
      await dureesString.forEach(duree => {
        let valeurString = 0;
        if (duree.nom === '0-30') {
        ...
        tempsAccompagnement.find(duree => duree.nom === activite.nom).valeur += valeurString;
      });
    }
  });
  console.log(tempsAccompagnement);
  //Temps en minutes
...
module.exports = { getStatsTempsAccompagnement };
